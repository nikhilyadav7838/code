...
public class MobUIAction extends BaseTest{
	...
	private static String timeInSecond;
     ...
     * @param isAMP				: true | False
     */
	public void setRequestParamOfGAandScoreCardForCricketCountry(List<LogEntry> entries, String eventCalls, String urlAttribute, String type, String action, boolean isAMP) {
		List<String> list = getListOfNetworkURLs(entries, eventCalls, urlAttribute, type, action);
		Map<String, String> map = setRequestParams(list);
		if(eventCalls.contains(Constants.GA_EVENT_NAME)) {
			googleAnalyticCalls = new GoogleAnalyticCalls(map.get(Constants.T), null, map.get(Constants.DL), map.get(Constants.TID), null);
		}else if(eventCalls.contains(Constants.SCORECARD_EVENT_NAME) && isAMP) {
			scoreCardCalls = new ScoreCardCalls(map.get(Constants.C2), map.get(Constants.C7), map.get(Constants.C8), map.get(Constants.C7_AMP));
		}else {
			System.out.println(map);
			scoreCardCalls = new ScoreCardCalls(map.get(Constants.C2), map.get(Constants.C7), null);
		}
	}
	...
	public int getStatusCodeOfURL(String url) {
		int statusCode = 0;
		...
		try {
			...
			double responeTime = ((endTime - startTime) / 1000.0000) % 60;
			timeInSecond = String.valueOf(responeTime);
		}
		return statusCode;
	}
